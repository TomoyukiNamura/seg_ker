...
tmp = tmp_raw0_median_diff[milage]

Q3 = tmp.dropna().quantile(.75)
Q1 = tmp.dropna().quantile(.25)

# 4分位範囲計算
quantile_range = (Q3 - Q1) * 2.0
...
# 異常値の除去
df_pred_raw_lm = ARIMA_funcs.postTreat(df_pred_raw=df_pred_raw_lm, abnormal_total=abnormal_total, start_raw_dict=start_raw_dict, t_pred=t_pred+lag_t, method=method_post)
...
ARIMA_funcs.plotTotalMAE(mae_dict=mae_dict_lm, ylim=[0.0, 1.0], r_plot_size=1, output_dir=f"{folder_name}/ARIMA")
...
## 予測モデル作成・逐次予測===============================================================
milage_list_list = ARIMA_funcs.makeMilageListList(n_state=n_state, stride=stride, tol_n_raw=tol_n_raw, n_org_train_dict=n_org_train_dict)
df_pred_raw_NN = ARIMA_funcs.predWithSpatialAriNnet(milage_list_list=milage_list_list, train_dict=train_dict, start_raw_dict=start_raw_dict, start_diff_dict=start_diff_dict, n_diff=n_diff, start_date_id=start_date_id, t_pred=t_pred+lag_t, batch_size=batch_size, epochs=epochs)
