...
def priorRawData(df_raw, window, min_periods, center, tol_diff=0.7, tol_n_group=5):
    ...
    for milage in tqdm(list(df_result.columns)):
        # 四分位範囲によるtol_diffの計算
        Q3 = tmp_raw0_median_diff[milage].dropna().quantile(.75)
        Q1 = tmp_raw0_median_diff[milage].dropna().quantile(.25)
        tol_diff = (Q3 - Q1) * 2.0
        
        # 差分がtol_diffより大きいところを取得
        tmp_raw0_median_diff_over_tol_diff = np.logical_or(tmp_raw0_median_diff[milage] > tol_diff, tmp_raw0_median_diff[milage].isna())
        
        # 10日間の差分がtol_diffより大きいidを取得
        over_tol_diff_id = pd.Series(list(tmp_raw0_median_diff[milage][tmp_raw0_median_diff_over_tol_diff].index))
        ...
        if len(group_list) > 0:
            ...
            displacement_point_list.append(group_list[i][len(group_list[i])-1]+1)
    ...
    #        plt.plot(tmp_raw)
    #        plt.plot(tmp_raw.rolling(window=20, min_periods=1, center=True).median())
    #        plt.plot(tmp_raw_local_1)
    #        plt.plot(tmp_raw_local_2)
            
            # 原系列を補正
            for i in range(len(displacement_point_list)-2):
                # グループ1,2を取得
                tmp_raw_local_1 = tmp_raw.iloc[range(displacement_point_list[i+1]),:]
