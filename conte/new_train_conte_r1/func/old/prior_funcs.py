...
def priorRawData(df_raw, window, min_periods, center, tol_diff=0.7, tol_n_group=5):
    ...
    # 10日間の差分
    date_diff = 10
    tmp_raw0_median_diff = np.abs(tmp_raw0_median.diff(date_diff))
    
    # 差分がtol_diffより大きい，または欠損しているところをTrue
    ...
    for milage in tqdm(list(df_result.columns)):
        ...
        # 差分が計算できない前10日間分のidを除外
        over_tol_diff_id = over_tol_diff_id.iloc[range(date_diff,over_tol_diff_id.shape[0])].reset_index(drop=True)        
        ...
        # グループの長さがtol_n_group以上のもののみにする
        org_group_list = deepcopy(group_list)
        for i in range(len(org_group_list)): 
            if len(org_group_list[i]) < tol_n_group:
                group_list.remove(org_group_list[i])
        ...
        if len(group_list) > 0:
            ...
            # 原系列を補正
            for i in range(len(displacement_point_list)):
                # 基準となる原系列を取得
                reference_raw = tmp_raw.iloc[range(displacement_point_list[i]),:]
                
                # 補正対象となる原系列を取得
                target_raw = tmp_raw.iloc[range(displacement_point_list[i], tmp_raw.shape[0]),:]
                
                # reference_rawとtarget_raw両方に何らかのに値があれば，補正を実行
                if len(reference_raw.dropna())!=0 and len(target_raw.dropna())!=0:
                    # 基準の末尾20中央値，と対象の先頭20中央値を算出
                    ave_reference = float(np.median(reference_raw.dropna().tail(20)))
                    ave_target    = float(np.median(target_raw.dropna().head(20)))
                    
                    # 中央値差を対象原系列に足し，保存
                    tmp_raw.iloc[range(displacement_point_list[i], tmp_raw.shape[0]),:] = (ave_reference - ave_target) + target_raw
    ...
    df_result2 = []
    for milage in tqdm(list(df_result.columns)):
        tmp = deepcopy(df_result.loc[:,[milage]].dropna())
        tmp.index = pd.RangeIndex(start=df_result.shape[0]-tmp.shape[0] ,stop=df_result.shape[0] ,step=1)
        df_result2.append(tmp)
        
    df_result2 = pd.concat(df_result2,axis=1)
    df_result = df_result2
                
            
    # 直線補完
    # df_result = df_result.interpolate(limit=3, limit_direction='both')
