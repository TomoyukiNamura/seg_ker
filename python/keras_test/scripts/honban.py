...
#    df_irregularity_phase_modified.shape
    ...
    # 原系列の前処理：移動平均
    org_dict["raw0_prior_treated"], _, _  = prior_funcs.priorRawData(df_raw=org_dict["raw0"], window=window, min_periods=min_periods, center=center, tol_diff=0.7, tol_n_group=5)
    ...
    if output_movie:
        ...
        org_dict["raw0"].to_csv(f"{movie_folder_name}/raw0.csv",index=False,header=True)
        org_dict["raw0_prior_treated"].to_csv(f"{movie_folder_name}/raw0_prior_treated.csv",index=False,header=True)
        train_dict["raw0"].to_csv(f"{movie_folder_name}/train.csv",index=True,header=True)
        train_dict["raw0_prior_treated"].to_csv(f"{movie_folder_name}/train_prior_treated.csv",index=True,header=True)
